          ...
          <div class="row">
            ...
            <div class="row mt-4" style="justify-content: right;">
              <div class="col-6" style="display: flex">
                ...
                <div class="col-6">
                  <button id="btn-white" @click="updateManut"  class="btn w-100 px-3 mb-2 btn-outline-success">Salvar</button>
                </div>
              </div>
            </div>
          </div>
...
import {editManutencao, getInfosManut} from "@/views/Manutenções/manutencoes_service";
import {formatDate} from "@/utils";

export default {
  name: "edit_manutencao",
  components: {
    ...
    getAllInfosManutencao() {
      getInfosManut(this.ID, localStorage.getItem('estabId'))
          .then((response) => {
            ...
            this.forms[1].value = formatDate(response.data.data.data_ocorrencia);
            this.forms[2].value = response.data.data.servico;
            this.forms[3].value = response.data.data.tipo;
            this.forms[4].value = response.data.data.descricao;
